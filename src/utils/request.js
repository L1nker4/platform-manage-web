...
// 拦截请求
FOREST_REQUEST.interceptors.request.use((config) => {
  ...
  // 有 token就带上
  if (store.state.account.token && typeof store.state.account.token === 'string') {
...
// 拦截响应
FOREST_REQUEST.interceptors.response.use((config) => {
  return config
}, (error) => {
  if (error.response) {
...
const request = {
  ...
  get (url, params) {
    ...
    return FOREST_REQUEST.get(`${url}${_params}`)
  },
  delete (url, params) {
    let _params
    if (Object.is(params, undefined)) {
    ...
    return FOREST_REQUEST.delete(`${url}${_params}`)
  },
  export (url, params = {}) {
